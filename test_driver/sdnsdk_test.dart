...
void main() => group('Integration tests', () {
      test('E2EE', () async {
        ...
        try {
          ...
          final nodeUri = Uri.parse(node);
          Logs().i('++++ Using node $nodeUri ++++');

          Logs().i('++++ Login Alice at ++++');
          testClientA = Client('TestClientA', databaseBuilder: getDatabase);
          await testClientA.checkNode(nodeUri);
          ...
          await testClientB.checkNode(nodeUri);
  ...
  expect(room.client.encryption.keyManager.getInboundGroupSession(
      ...
      null);*/
          ...
          await testClientC.checkNode(nodeUri);
          // We can't sign in using the displayname, since that breaks e2ee on radix: https://github.com/sdn-org/radix/issues/2914
      ...
      null);*/
          ...
          if (Platform.environment['NODE_IMPLEMENTATION'] != 'conduit') {
            expect(
                room.client.encryption!.keyManager
                    .getOutboundGroupSession(room.id)!
                    .outboundGroupSession!
...
Object get olmLengthMatcher {
  return
      // workarounding weird
      // bug
      Platform.environment['NODE_IMPLEMENTATION'] != 'radix'
          ? 1
          : predicate(
              [1, 2].contains,
              'is either 1 or two',

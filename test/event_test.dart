...
void main() {
  ...
  group('Event', () {
    ...
    final client = Client('testclient', httpClient: FakeSDNApi());
    ...
    test('sendAgain', () async {
      final sdn = Client('testclient', httpClient: FakeSDNApi());
      await sdn.checkNode(Uri.parse('https://fakeserver.notexisting'),
          checkWellKnown: false);
      await sdn.login(LoginType.mLoginPassword,
          identifier: AuthenticationUserIdentifier(user: 'test'),
          password: '1234');

      final event =
          Event.fromJson(jsonObj, Room(id: '!1234:example.com', client: sdn));
    ...
    test('requestKey', () async {
      ...
      }, Room(id: '!1234:example.com', client: sdn));
    ...
    test('getLocalizedBody, isEventKnown', () async {
      final sdn = Client('testclient', httpClient: FakeSDNApi());
      final room = Room(id: '!1234:example.com', client: sdn);
      ...
      expect(await event.calcLocalizedBody(SDNDefaultLocalizations()),
          'Removed by Example');
      ...
      event = Event.fromJson({
        'content': {
          ...
          'info': {
            ...
            'thumbnail_url': 'mxc://sdn.org/sHhqkFCvSkFwtmvtETOtKnLP',
            'w': 140
          },
          'url': 'mxc://sdn.org/sHhqkFCvSkFwtmvtETOtKnLP'
        },
        'event_id': '\$143273582443PhrSn:example.org',
        'origin_server_ts': 1432735824653,
        'room_id': '!jEsUZKDJdhlrceRyVU:example.org',
    ...
    test('getLocalizedBody, parameters', () async {
      ...
      expect(
          await event.calcLocalizedBody(SDNDefaultLocalizations(),
              plaintextBody: true),
          '**This is an example text message**');
    ...
    test('attachments', () async {
      ...
      Future<Uint8List> downloadCallback(Uri uri) async {
        return {
          '/_api/media/v3/download/example.org/file': FILE_BUFF,
          '/_api/media/v3/download/example.org/thumb': THUMBNAIL_BUFF,
        }[uri.path]!;
      }

      await client.checkNode(Uri.parse('https://fakeserver.notexisting'),
          checkWellKnown: false);
      ...
      expect(event.getAttachmentUrl().toString(),
          'https://fakeserver.notexisting/_api/media/v3/download/example.org/file');
      expect(event.getAttachmentUrl(getThumbnail: true).toString(),
          'https://fakeserver.notexisting/_api/media/v3/thumbnail/example.org/file?width=800&height=800&method=scale&animated=false');
      expect(event.getAttachmentUrl(useThumbnailMxcUrl: true).toString(),
          'https://fakeserver.notexisting/_api/media/v3/download/example.org/thumb');
      expect(
          event
              .getAttachmentUrl(getThumbnail: true, useThumbnailMxcUrl: true)
              .toString(),
          'https://fakeserver.notexisting/_api/media/v3/thumbnail/example.org/thumb?width=800&height=800&method=scale&animated=false');
      expect(
          event
    ...
    test('encrypted attachments', () async {
      ...
      Future<Uint8List> downloadCallback(Uri uri) async {
        return {
          '/_api/media/v3/download/example.com/file': FILE_BUFF_ENC,
          '/_api/media/v3/download/example.com/thumb': THUMB_BUFF_ENC,
        }[uri.path]!;
      }
    ...
    test('downloadAndDecryptAttachment store', () async {
      ...
      Future<Uint8List> downloadCallback(Uri uri) async {
        serverHits++;
        return {
          '/_api/media/v3/download/example.org/newfile': FILE_BUFF,
        }[uri.path]!;
      }

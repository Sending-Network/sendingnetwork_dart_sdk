...
/// https://sdn.to/#/!KBwfdofYJUmnsVoqwn:famedly.de/$wlHXlLQJdfrqKAF5KkuQrXydwOhY_uyqfH4ReasZqnA?via=neko.dev&via=famedly.de&via=lihotzki.de
...
class KeyVerification {
  ...
  static const String prefix = 'SDN';
  ...
  Future<QRCode?> generateQrCode() async {
    ...
    // why 11? https://github.com/sdn-org/sdn-js-sdk/commit/275ea6aacbfc6623e7559a7649ca5cab207903d9
...
class _KeyVerificationMethodSas extends _KeyVerificationMethod {
  ...
  Uint8List makeSas(int bytes) {
    ...
    if (keyAgreementProtocol == 'curve25519-hkdf-sha256') {
      ...
      sasInfo =
          'SDN_KEY_VERIFICATION_SAS|${request.startedVerification ? ourInfo + theirInfo : theirInfo + ourInfo}${request.transactionId!}';
  ...
  Future<void> _sendMac() async {
    final baseInfo =
        'SDN_KEY_VERIFICATION_MAC${client.userID!}${client.deviceID!}${request.userId}${request.deviceId!}${request.transactionId!}';
  ...
  Future<void> _processMac() async {
    ...
    final baseInfo =
        'SDN_KEY_VERIFICATION_MAC${request.userId}${request.deviceId!}${client.userID!}${client.deviceID!}${request.transactionId!}';

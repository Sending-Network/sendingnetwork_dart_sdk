...
/// Represents a SDN client to communicate with a
/// [SDN](https://sdn.org) node and is the entry point for this
/// SDK.
class Client extends SDNApi {
  int? _id;
  ...
  /// client and sets [node] to [nodeUrl] if it is. Supports the
  ...
  /// Returns the fully-qualified SDN user ID (MXID) that has been registered.
  /// You have to call [checkNode] first to set a node.
  @override
  ...
  Future<SDNDIDListResponse> getDIDList({String? address}) async {
    final response = await super.getDIDList(
      address: address,
    );
  ...
  /// Sends a logout command to the node and clears all local data,
  ...
  // https://sdn-org.github.io/synapse/latest/usage/configuration/config_documentation.html#caches-and-associated-values
  ...
  ///
  /// Before you can connect you need at least an [accessToken], a [node],
  ...
  Future<void> clear() async {
    ...
    _id = accessToken = syncFilterId =
        node = _userID = _deviceID = _deviceName = prevBatch = null;
    _rooms = [];
  ...
  Future<void> _handleSync(SyncUpdate sync, {Direction? direction}) async {
    ...
    for (final newPresence in sync.presence ?? []) {
      final cachedPresence = CachedPresence.fromSDNEvent(newPresence);
  ...
  Future<void> processToDeviceQueue() async {
    ...
    for (final entry in entries) {
      ...
      // https: //sdn.org/docs/spec/client_server/r0.6.1#put-sdn-client-r0-sendtodevice-eventtype-txnid
  ...
  /// rule of the push rules: https://sdn.org/docs/spec/client_server/r0.6.0#m-rule-master
...
class FileTooBigSDNException extends SDNException {
  int actualFileSize;
  int maxFileSize;

...
extension SDNIdExtension on String {
  ...
  /// Parse a sdn identifier string into a Uri. Primary and secondary identifiers
  /// are stored in pathSegments. The query string is stored as such.
  Uri? _parseIdentifierIntoUri() {
    const sdnUriPrefix = 'sdn:';
    const sdnToPrefix = 'https://sdn.to/#/';
    if (toLowerCase().startsWith(sdnUriPrefix)) {
    ...
    } else if (toLowerCase().startsWith(sdnToPrefix)) {
      return Uri.tryParse(
          '//${substring(sdnToPrefix.length - 1).replaceAllMapped(RegExp(r'(?<=/)[#!@+][^:]*:|(\?.*$)'), (m) => m[0]!.replaceAllMapped(RegExp(m.group(1) != null ? '' : '[/?]'), (m) => Uri.encodeComponent(m.group(0)!))).replaceAll('#', '%23')}');
    } else {
      return Uri(
  ...
  /// a query string and already parsed `via` parameters. A sdn identifier string
...
class SDNIdentifierStringExtensionResults {

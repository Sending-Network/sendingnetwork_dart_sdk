...
class FakeSDNApi extends BaseClient {
  ...
  FutureOr<Response> mockIntercept(Request request) async {
    ...
    if (request.url.path.contains('/_api')) {
      action = '${request.url.path.split('/_api').last}?${request.url.query}';
    }
...
//        },
  ...
  final Map<String, Map<String, dynamic>> api = {
    'GET': {
      ...
      '/media/v3/preview_url?url=https%3A%2F%2Fsdn.org&ts=10': (var req) => {
            'og:title': 'SDN Blog Post',
            'og:description': 'This is a really cool blog post from sdn.org',
      ...
      '/.well-known/sdn/client': (var req) => {
            'm.node': {'base_url': 'https://sdn.example.com'},
      ...
      '/client/v3/thirdparty/location?alias=1234': (var req) => [
            {
              'alias': '#freenode_#sdn:sdn.org',
              'protocol': 'irc',
              'fields': {'network': 'freenode', 'channel': '#sdn'}
            }
          ],
      ...
      '/client/v3/directory/room/%23testalias%3Aexample.com': (var reqI) => {
            ...
            'servers': ['capuchins.com', 'sdn.org', 'another.com']
          },
            ...
            'pushers': [
              {
                ...
                'lang': 'en-US',
                'data': {
                  'url': 'https://example.com/_api/push/v1/notify',
      ...
      '/client/unstable/org.sdn.msc3814.v1/dehydrated_device': (var _) => {
            'device_id': 'DEHYDDEV',
            'device_data': {'algorithm': 'some.famedly.proprietary.algorithm'},
          },
      '/client/unstable/org.sdn.msc3814.v1/dehydrated_device/DEHYDDEV/events?limit=100':
          (var _) => {
                'events': [
                  {
                    // this is the commented out m.room_key event - only encrypted
    ...
    'POST': {
      ...
      '/client/v3/login': (var req) => {
            ...
            'device_id': 'GHTYAJCE',
            'well_known': {
              'm.node': {'base_url': 'https://example.org'},
    ...
    'PUT': {
      ...
      '/client/v3/rooms/!localpart%3Aserver.abc/send/m.call.sdp_stream_metadata_changed/1234':
          (var req) => {'event_id': '1234'},
      '/client/v3/rooms/!localpart%3Aserver.abc/send/org.sdn.call.sdp_stream_metadata_changed/1234':
          (var req) => {'event_id': '1234'},
      '/client/v3/rooms/!localpart%3Aserver.abc/send/org.sdn.call.asserted_identity/1234':
          (var req) => {'event_id': '1234'},
      ...
      '/client/v3/rooms/!localpart%3Aserver.abc/send/m.room.message/testtxid':
          (var reqI) => {
                'event_id': '\$event${FakeSDNApi.eventCounter++}',
              },
      ...
      '/client/v3/rooms/${Uri.encodeComponent("!localpart:server.abc")}/state/${Uri.encodeComponent("org.sdn.msc3401.call")}/${Uri.encodeComponent("1675856324414gzczMtfzTk0DKgEw")}':
          (var req) => {
                'event_id': 'groupCall',
              },
    },
